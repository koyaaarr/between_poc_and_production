...
![organization](./images_en/organization.png)
...
![architecture_poc](./images_en/architecture_poc.png)
...
Use LightGBM to create a model. Also, use [Optuna](https://github.com/optuna/optuna) to tune hyperparameters.
...
For managing machine learning models, [MLflow](https://github.com/mlflow/mlflow) is useful. It can manage models with each hyperparameter explored by Optuna, which will be useful as the number of model trials increases.
...
For the GUI, we need a button to execute the web server API and a form to upload data. In this case, I used [React](https://github.com/facebook/react) and [Typescript](https://github.com/microsoft/TypeScript) to create the GUI on my own, but it may be faster to use a library that creates the GUI, such as [streamlit](https://github.com/streamlit/streamlit).
